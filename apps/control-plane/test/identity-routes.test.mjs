...
test("device registration derives ownership server-side and requires proof", async () => {
  let command;
  const app = Fastify({logger: false});
  registerIdentityRoutes(app, {
    establishSession: async () => { throw new Error("unused"); },
  ...
  assert.equal(response.statusCode, 201);
  assert.equal(command.tenantId, "22222222-2222-4222-8222-222222222222");
  assert.equal(command.userId, "55555555-5555-4555-8555-555555555555");
...
test("session termination cannot target another user's session", async () => {
  ...
  const response = await app.inject({method: "DELETE", url: "/v1/identity/sessions/10101010-1010-4010-8010-101010101010", payload: {
